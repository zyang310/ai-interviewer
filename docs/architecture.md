...
**Built (Phase 3 — global voice hotkey):** in addition to the click-to-toggle mic, a configurable global hotkey **toggles** recording — press once to start, press again to stop & send (same as the mic button). A backend OS-level keyboard hook (`internal/hotkey`, `robotn/gohook`) runs on a libuiohook C thread and emits a `ptt:down` Wails event per press; `App.tsx` subscribes via `EventsOn`, toggling the existing `useVoiceRecorder` on each press and barging in over any TTS when recording starts. The hook is **passive** — it observes the keystroke but does not consume it, so the key still reaches the focused IDE (pick one it ignores; a tapped combo can type, fire shortcuts, or ring the macOS key beep). It works regardless of window focus and on both macOS and Windows; **macOS requires Accessibility permission** (`AXIsProcessTrustedWithOptions`; no programmatic grant — surfaced via `GetHotkeyStatus` + `OpenAccessibilitySettings` in Settings; the dialog is asked once ever, and a grant is picked up live on window focus via `RetryHotkey`, no relaunch). The auto-repeat debounce (one `ptt:down` per physical press) lives in a pure, unit-tested matcher; `keymap.go` maps the canonical hotkey string ↔ gohook keycodes ↔ display label. **Cross-compile caveat:** gohook needs CGO + native toolchains, so build each OS natively (or via a CI matrix) rather than cross-compiling.
...
func (a *App) RetryHotkey()                      // silent re-apply on window focus — picks up an Accessibility grant live (no relaunch)
func (a *App) OpenAccessibilitySettings()       // macOS: open System Settings → Privacy & Security → Accessibility
```
...
- **Distribution & updates** (macOS, signed + notarized): CI builds on every `main` push; `vX.Y.Z` tags publish a public GitHub Release; on launch the app checks for a newer release (`internal/updater`, bound via `CheckForUpdate`/`GetAppVersion`) and shows a banner whose click self-installs it — `InstallUpdate` downloads, verifies the signature, swaps the bundle via a detached helper, and restarts the app — falling back to `OpenReleasePage` if that isn't possible. Full explainer: [ci-cd-and-auto-update.md](ci-cd-and-auto-update.md).
