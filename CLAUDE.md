...
| `window_space_darwin.go` | Darwin-only cgo/Cocoa path that sets the overlay window's `NSWindow.collectionBehavior` (`canJoinAllSpaces` + `fullScreenAuxiliary`) so it floats over another app's **full-screen** Space — Wails exposes always-on-top (a window level) but not collection behavior, and level alone can't reach a full-screen Space. Toggled by `EnterOverlayMode`/`ExitOverlayMode`; `window_space_other.go` stubs it (Windows always-on-top already floats over full-screen; X11 has no Spaces model). |
